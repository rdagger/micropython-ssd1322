...
class BouncingText(object):
    ...
    def draw(self):
        ...
        self.display.draw_text(x, y, self.text, self.font, gs=self.gs)
...
def test():
    ...
    try:
        ...
        # Load text object
        bouncing_text = BouncingText('256x64 OLED Display',
                                     unispace, 256, 64, 1, display)

        while True:
            timer = ticks_us()
            bouncing_text.update_pos()
            bouncing_text.draw()
